 ...
 */
 ...
 function replaceCSSComments(comment) {
...
function checkYear(year) {
    return (/\b(19[0-9]{2}|20[0-9]{2}|2100)\b/).test(year);
}
 ...
 */
 ...
 function checkEmail(email) {
    return (/^[a-z]+\W?[a-z]+@[a-z\.]+\.[a-z]{2,3}$/).test(email);
 }
...
function createLinksFromDomains (str) {
  ...
  var item_reg = new RegExp('^(http:|https:)\\/\\/','gi');
  str.match(reg).forEach((item) => str = str.replace(item, `<a href="${item}" target="_blank">${item.replace(item_reg, '')}</a>`));
